...
pub mod tokenvesting {
    ...
    /// Used by the company to store informations
    ...
    /// Used to create an employee account with vesting information
    ...
    pub fn claim_tokens(ctx: Context<ClaimTokens>, _company_name: String) -> Result<()> {
        ...
        // Calculate time since employee vesting time has started
        let time_since_start = now.saturating_sub(employee_account.start_time); // "saturating" prevent overflow error

        // Employee total vesting time 
        ...
        // If employee vesting time is ended, return total amount 
        // Else calculate amount that user can claim
        ...
        let signer_seeds: &[&[&[u8]]] = &[
          &[b"vesting_treasury", ctx.accounts.vesting_account.company_name.as_ref(),
